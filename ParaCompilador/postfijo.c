...
void pretraducir(char *origS, char *newS) {
	...
	while(origS[i] != '\0') {
		...
		if(strncmp(&origS[i], "resComp", strlen("resComp"))==0) {
			strncpy(&newS[j], "resComp", strlen("resComp"));
			j+=strlen("resComp");
			i+=strlen("resComp");
		}
		else
		if(strncmp(&origS[i], "res", strlen("res"))==0 || strncmp(&origS[i], "ans", strlen("ans"))==0) {
		...
		if(strncmp(&origS[i], "sqrt", strlen("sqrt"))==0) {
			newS[j++]='a';
			i+= strlen("sqrt");
		}
		else
		if(strncmp(&origS[i], "sin",  strlen("sin"))==0 || strncmp(&origS[i], "sen",  strlen("sen"))==0) {
			newS[j++]='b';
			i+=strlen("sin");
		}
		else
		if(strncmp(&origS[i], "cos", strlen("cos"))==0 ) {
			newS[j++]='c';
			i+=strlen("cos");
		}
		else
		if(strncmp(&origS[i], "tan", strlen("tan"))==0 ) {
			newS[j++]='d';
			i+=strlen("tan");
		}
		else
		if(strncmp(&origS[i], "asin", strlen("asin"))==0  || strncmp(&origS[i], "asen", strlen("asen"))==0) {
			newS[j++]='e';
			i+=strlen("asin");
		}
		else
		if(strncmp(&origS[i], "acos", strlen("acos"))==0) {
			newS[j++]='f';
			i+=strlen("acos");
		}
		else
		if(strncmp(&origS[i], "atan", strlen("atan"))==0) {
			newS[j++]='g';
			i+=strlen("atan");
		}
		else    
		if(strncmp(&origS[i], "exp", strlen("exp"))==0) {
			newS[j++]='h';
			i+=strlen("exp");
		}
		else
		if(strncmp(&origS[i], "ln", strlen("ln"))==0) {
			newS[j++]='z';
			i+=strlen("ln");
		}
		else
		if(strncmp(&origS[i], "log", strlen("log"))==0) {
			newS[j++]='k';
			i+=strlen("log");
		}
		else
		if(strncmp(&origS[i], "sh", strlen("sh"))==0) {
			newS[j++]='l';
			i+=strlen("sh");
		}
		else
		if(strncmp(&origS[i], "ch", strlen("ch"))==0) {
			newS[j++]='m';
			i+=strlen("ch");
		}
		else
		if(strncmp(&origS[i], "th", strlen("th"))==0) {
			newS[j++]='n';
			i+=strlen("th");
		}
		else
		if(strncmp(&origS[i], "ash", strlen("ash"))==0) {
			newS[j++]='o';
			i+=strlen("ash");
		}
		else
		if(strncmp(&origS[i], "ach", strlen("ach"))==0) {
			newS[j++]='p';
			i+=strlen("ach");
		}
		else
		if(strncmp(&origS[i], "ath", strlen("ath"))==0) {
			newS[j++]='q';
			i+=strlen("ath");
		}
		else
		if(strncmp(&origS[i], "pow", strlen("pow"))==0) {
			newS[j++]='r';
			i+=strlen("pow");
		}
		else
		if(strncmp(&origS[i], "gamma'", strlen("gamma'"))==0) {
			newS[j++]='w';
			i+=strlen("gamma'");
		}
		else
		if(strncmp(&origS[i], "factorial", strlen("factorial"))==0) {
			newS[j++]='t';
			i+=strlen("factorial");
		}
		else
		if(strncmp(&origS[i], "betha", strlen("betha"))==0) {
			newS[j++]='u';
			i+=strlen("betha");
		}
		else
		if(strncmp(&origS[i], "prim", strlen("prim"))==0) {
			newS[j++]='v';
			i+=strlen("prim");
		}
		else
		...
		if(strncmp(&origS[i], "pi", strlen("pi"))==0) {
			newS[j]='U';
			j++;
			i+=strlen("pi");
		}
		else
		if(strncmp(&origS[i], "e", strlen("e"))==0) {
			newS[j]='V';
			j++;
			i+=strlen("e");
		}
		else
		if(strncmp(&origS[i], "r2", strlen("r2"))==0) {
			newS[j]='W';
			j++;
			i+=strlen("r2");
		}
...
double eval_func(char *s, double var[]) {
	...
	opndstk=NULL;
	while(s[i]!= '\0') {
		if(strncmp(&s[i],"res",strlen("res"))==0) {
			push(&opndstk, res);
			i+=strlen("res");
		}
		else
...
complex eval_funcComp(char *s, double var[]) {
	...
	while(s[i]!= '\0')
	{
		...
		if(strncmp(&s[i],"resComp",strlen("resComp"))==0)
		{
