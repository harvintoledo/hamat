...
class LoggerPrivate
{
  public:
    ...
    {
      bool lNoDebugOutPut = false;

#if defined(QT_NO_DEBUG_STREAM)
      lNoDebugOutPut = true;
#endif

      if(!(lNoDebugOutPut && logLevel == Logger::Debug))
      {
        ...
        {
          // Fallback
          QString result = QString(QLatin1String("[%1] <%2>:%3 %4")).arg(Logger::levelToString(logLevel), -7)
                                                                 .arg(function).arg(line).arg(message);
